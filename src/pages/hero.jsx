...
import a2 from "../images/contact.png";
...
export default function Hero() {
  ...
  const navItems = [
    ...
    {
      label: "COMMUNITY",
      submenu: ["CSR", "Knowledge Hub", "Awards and Achievements", "Blogs", "FAQ"],
    },
    {
      label: "SUSTAINBLITY",
    ...
    <div className="absolute left-0 mt-2 bg-black/90 backdrop-blur-md text-white rounded-md shadow-lg p-4 w-full sm:w-[700px] md:w-[800px] z-50 border border-orange-500">
      <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-orange-500 gap-4">
        <div className="space-y-2 px-2">
          {["Steel", "Power", "Aluminium", "SEL Tiger", "View All"].map((item, i) => (
            <div key={i} className="cursor-pointer hover:text-orange-500 select-text">{item}</div>
          ))}
        </div>
        <div className="space-y-2 px-2">
          {["Carbon Steel", "Cold Rolled", "Stainless Steel", "Special Alloys"].map((item, i) => (
            <div key={i} className="cursor-pointer hover:text-orange-500 select-text">{item}</div>
          ))}
        </div>
          ...
          {[img3, img4, img5].map((img, row) => (
            <div key={row} className="flex gap-2">
              {[0, 1, 2].map((col) => {
                ...
                return (
                  <div key={col} className="flex flex-col items-center text-center w-20">
                    <img src={img} alt="item" className="w-12 h-12 object-contain" />
                    <span className="text-xs mt-1 select-text">{labels[row][col]}</span>
                  </div>
                );
              })}
            </div>
  ...
  return (
    ...
    >
          ...
          <ol className="flex flex-wrap items-center justify-center gap-6 text-sm sm:text-base relative z-50">
            ...
            {navItems.map((item, index) => (
              <li key={index} className="relative group">
                ...
                >
                  {item.label}
                  {(item.submenu && item.submenu.length > 0) || item.label === "BUSINESS" ? (
                    <ChevronDown className="w-4 h-4" />
                  ) : null}
                </div>

                {item.label === "BUSINESS" && showBusinessMenu && <BusinessSection />}

                {item.label !== "BUSINESS" && item.submenu && item.submenu.length > 0 && (
      ...
      <div className="lg:hidden flex items-center justify-between px-6 py-4">
        <img src={a1} alt="Logo" className="h-8 w-auto" />
        <button aria-label="Toggle menu" onClick={() => setSidebarOpen(true)} className="text-white">
          <Menu className="w-6 h-6" />
        </button>
      </div>

      {sidebarOpen && <div onClick={closeSidebar} className="fixed inset-0 bg-black bg-opacity-50 z-40" />}

      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-md text-black z-50 transform transition-transform duration-300 ease-in-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <nav className="flex flex-col p-6 space-y-4 text-lg font-semibold select-none">
          {navItems.map((item, idx) => (
            <div key={idx} className="flex flex-col">
              ...
              {item.submenu && item.submenu.length > 0 && openSubmenuIndex === idx && (
      ...
      <div className="px-9 mt-30 max-w-2xl w-full ml-auto lg:ml-12 lg:w-1/2 text-left">
        ...
        <br />
        <h3>
          Fusce et diam nisl. Curabitur est orci, tempus nec iaculis non, hendrerit eget est. Fusce nisi lorem,
          scelerisque vitae tempus eget, consequat ultracies nulls
        </h3>
        <hr
